...
dwordVal DWORD 100000
sdwordVal SDWORD -100000

fwordVal FWORD 12345678h

qwordVal QWORD 123456789ABCDEF0h

tbyteVal TBYTE 12345678901234567890h
...
HelloText    TEXTEQU <"Hello">
AssemblyText TEXTEQU <"Assembly Language">
CourseText   TEXTEQU <"Computer Science">

.data

message1 BYTE HelloText, 0
message2 BYTE AssemblyText, 0
message3 BYTE CourseText, 0
...
AddTwoSum의 listing file을 분석하면 각 instruction 뒤에 assembler가 생성한 machine code가 표시된다. mov eax,10000h의 machine code는 B8 00 00 01 00이다. 여기서 B8은 EAX에 immediate 값을 이동시키는 MOV opcode이며, 뒤의 00 00 01 00은 little-endian 형식으로 저장된 00010000h이다.

add eax,40000h의 machine code는 05 00 00 04 00이다. 05는 EAX에 immediate 값을 더하는 ADD opcode이고, 뒤의 4 bytes는 00040000h를 little-endian으로 나타낸 것이다.

sub eax,20000h의 machine code는 2D 00 00 02 00이다. 2D는 EAX에서 immediate 값을 빼는 SUB opcode이며, 뒤의 4 bytes는 00020000h이다.

call DumpRegs는 E8로 시작한다. E8은 CALL instruction의 opcode이고, 뒤의 4 bytes는 호출 대상까지의 상대 주소(relative displacement)를 나타낸다. DumpRegs는 외부 라이브러리 프로시저이므로 assembler 단계에서는 최종 주소를 알 수 없어 해당 값의 의미를 정확히 해석하기 어려울 수 있다.

각 instruction은 이 경우 5 bytes를 차지하므로 instruction의 offset은 00000000, 00000005, 0000000A, 0000000F 순서로 증가한다.
...
먼저 기존 AddVariables 프로그램의 DWORD 변수를 QWORD로 변경하였다. 이후 기존의 32비트 레지스터인 EAX를 사용하여 QWORD 데이터를 처리하려 하자 operand size와 관련된 오류가 발생하였다. QWORD는 64비트 데이터이므로 64비트 환경에서 RAX와 같은 64비트 레지스터를 사용하도록 명령어를 수정하였다. 그 결과 64비트 변수에 대한 덧셈을 수행할 수 있었다.
